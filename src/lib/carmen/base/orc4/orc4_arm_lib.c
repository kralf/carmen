...
static void recover_failure(void)
{
  ...
  while (serial_fd < 0) {
    ...
    carmen_arm_direct_initialize(NULL);
  }
}

unsigned char arm_create_checksum(unsigned char *buffer, int size)
{
...
void arm_send_packet(unsigned char *byte, int length,  unsigned char where)
{
  ...
  for (i = 0; i < length; i++)
    buffer[i+3] = byte[i];
  buffer[length+3] = arm_create_checksum(buffer, length+3);
...
static unsigned char *read_packet(void)
{
  ...
  do {
    ...
    checksum = arm_create_checksum(buffer, packet_length-1);
...
static int arm_send_packet_and_ack(unsigned char *byte, int length,
			       unsigned char where)
{
  int count;
  int err;
  ...
  count = 0;
  do {
    arm_send_packet(byte, length, where);
    err = wait_for_ack();
    if (err == 0)
      return 0;
    count++;
           ...
           int current_pwm, unsigned char WHICH_MOTOR)
{
  ...
  arm_send_packet_and_ack(buffer, 4, ORC_SLAVE);
}
...
void carmen_arm_direct_set( double shoulder_desired_angle, double elbow_desired_angle, double gripper_desired_angle ) {
  ...
  buffer[2] = pwm >> 8;
  buffer[3] = pwm & 0x00ff;
  arm_send_packet_and_ack(buffer, 4, ORC_MASTER);
...
static void unpack_master_packet(unsigned char *buffer)
{
 ...
 //printf("unpack_master_packet\n");
  ...
  if (sonar_on && carmen_get_time() - last_ping_time > .05) {
    ...
    arm_send_packet_and_ack(command_buffer, 2, ORC_MASTER);
...
int carmen_arm_direct_initialize(carmen_arm_model_t *arm_model)
{
  ...
  result = carmen_serial_connect(&serial_fd, arm_model->dev);
  if(result == -1) {
    serial_fd = -1;
    return -1;
  }

  if (strstr(arm_model->dev, "USB") == NULL) {
  ...
  if (arm_send_packet_and_ack(buffer, 3, ORC_SLAVE) < 0) {
  ...
  if (arm_send_packet_and_ack(buffer, 3, ORC_MASTER) < 0) {
    fprintf(stderr, "%sfailed%s.\n", carmen_red_code, carmen_normal_code);
    return -1;
  }
  fprintf(stderr, "ok\n");
  ...
  if (arm_send_packet_and_ack(buffer, 4, ORC_SLAVE) < 0) {
    fprintf(stderr, "%sfailed%s.\n", carmen_red_code, carmen_normal_code);
    return -1;
  }
  fprintf(stderr, "ok\n");
...
int carmen_arm_direct_shutdown(void)
{
  ...
  buffer[3] = 0;
  arm_send_packet_and_ack(buffer, 4, ORC_SLAVE);  buffer[0] = 0x4D;
...
/*     buffer[2] = pwm >> 8; */
/*     buffer[3] = pwm & 0x00ff; */
/*     arm_send_packet(buffer, 4, ORC_MASTER); */
/*   } */
/* } */
