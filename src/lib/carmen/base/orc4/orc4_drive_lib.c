...
unsigned char drive_create_checksum(unsigned char *buffer, int size)
{
...
void drive_send_packet(unsigned char *byte, int length,  unsigned char where)
{
  ...
  for (i = 0; i < length; i++)
    buffer[i+3] = byte[i];
  buffer[length+3] = drive_create_checksum(buffer, length+3);
...
static unsigned char *read_packet(void)
{
  ...
  do {
    ...
    checksum = drive_create_checksum(buffer, packet_length-1);
...
static int drive_send_packet_and_ack(unsigned char *byte, int length,  
			       unsigned char where)
{
  int count;
  int err;
  ...
  count = 0;
  do {
    drive_send_packet(byte, length, where);
    err = wait_for_ack();
    if (err == 0)
      return 0;
				  ...
				  unsigned char WHICH_MOTOR)
{
  ...
  drive_send_packet_and_ack(buffer, 4, ORC_SLAVE);
}
...
static void unpack_master_packet(unsigned char *buffer)
{
  ...
  if (sonar_on && carmen_get_time() - last_ping_time > .05) {
    ...
    drive_send_packet_and_ack(command_buffer, 2, ORC_MASTER);
...
static int orc_set_encoder_quad_phase_fast(int motor) 
{
  ...
  fprintf(stderr, "Setting motor channel %d into quad phase fast mode... ",
	  motor);
  if (drive_send_packet_and_ack(buffer, 3, ORC_SLAVE) < 0) {
...
int orc_set_pin(int pin, char *description, int pin_mode) 
{
  ...
  fprintf(stderr, "Setting %s pin %d into %s mode... ", description, pin,
	  modes[pin_mode]);
  if (drive_send_packet_and_ack(buffer, 3, ORC_MASTER) < 0) {
    fprintf(stderr, "%sfailed%s.\n", carmen_red_code, carmen_normal_code);
    return -1;
  }
  fprintf(stderr, "ok.\n");
...
int orc_set_pwm(int motor, int dir, int pwm)
{
  ...
  buffer[2] = dir;
  buffer[3] = pwm;
  if (drive_send_packet_and_ack(buffer, 4, ORC_SLAVE) < 0) {
    return -1;
  }
...
static unsigned char *get_status_packet(unsigned char where)
{
  ...
  count = 0;
  do {
    drive_send_packet(&byte, 1, where);
    buffer = read_packet();
    if (buffer != NULL) {
      byte = buffer[2];
      routing = byte >> 6;
