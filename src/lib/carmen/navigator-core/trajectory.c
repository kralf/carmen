...
{
  carmen_traj_point_p new_points;

  if (path->capacity == 0) {
    path->capacity = 20;
    path->points = (carmen_traj_point_p)
      calloc(path->capacity, sizeof(carmen_traj_point_t));
    carmen_test_alloc(path->points);

  } else if (path->length == path->capacity) {
    path->capacity *= 2;

    new_points = (carmen_traj_point_p)realloc
      (path->points, sizeof(carmen_traj_point_t)*path->capacity);
...
{
  carmen_traj_point_p point, next_point;
...
int
carmen_planner_util_add_path_point(carmen_traj_point_t point,
...
void
carmen_planner_util_set_path_point(int index, carmen_traj_point_p path_point,
				   carmen_planner_path_p path)
{
  if (index >= path->length || index < 0)
    {
...
carmen_planner_util_insert_blank(int index, carmen_planner_path_p path)
{
  ...
  memmove(path->points+index+1, path->points+index,
	  num_to_move*sizeof(carmen_traj_point_t));
...
void
carmen_planner_util_set_path_velocities(int index, double t_vel,
					double r_vel, carmen_planner_path_p path)
{
  if (index >= path->length || index < 0)
    {
      carmen_warn("bad path id in %s : requested %d but max id is %d.\n",
...
carmen_planner_util_delete_path_point(int index, carmen_planner_path_p path)
{
  ...
  memmove(path->points+index,
	  path->points+index+1, num_to_move*sizeof(carmen_traj_point_t));
...
{
  int index;
  carmen_traj_point_t point = {0,0,0,0,0};
  carmen_traj_point_p point_p;
