...
RobotLaserMessage::RobotLaserMessage(const carmen_robot_laser_message& x) {
  m_msg = NULL;
  clone(x);
}

RobotLaserMessage::RobotLaserMessage(carmen_robot_laser_message* x) {
  m_msg = NULL;
...
void RobotLaserMessage::initRange(int num_readings) {
  ...
  if (num_readings>0) {
    m_msg->range = new float[num_readings];
...
void RobotLaserMessage::initRemission(int num_remissions) {
  ...
  if (num_remissions>0) {
    m_msg->remission = new float[num_remissions];
...
void RobotLaserMessage::setRobotLaserMessage( carmen_robot_laser_message* x) {
  m_msg = x;
}
...
void RobotLaserMessage::clone(const carmen_robot_laser_message& x) {
  this->free();
  m_msg = new carmen_robot_laser_message;
  carmen_test_alloc(m_msg);
  *m_msg = x;
  if (x.num_readings>0) {
    m_msg->range = new float[x.num_readings];
    carmen_test_alloc(m_msg->range);
  ...
  if (x.num_remissions>0) {
    m_msg->remission = new float[x.num_remissions];
    carmen_test_alloc(m_msg->remission);
  }
  else
    m_msg->remission = NULL;
...
const float* RobotLaserMessage::getRange() const{
  return m_msg->range;
}

const float* RobotLaserMessage::getRemission() const{
  return m_msg->remission;
}

float* RobotLaserMessage::getRange() {
  return m_msg->range;
}

float* RobotLaserMessage::getRemission() {
  return m_msg->remission;
}
...
float RobotLaserMessage::getRange(int i) const {
  return m_msg->range[i];
}

void RobotLaserMessage::setRange(int i, float val) {
  m_msg->range[i] = val;
}
...
float RobotLaserMessage::getRemission(int i) const {
  return m_msg->remission[i];
}

void RobotLaserMessage::setRemission(int i, float val) {
