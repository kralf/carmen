...
CarmenMap::CarmenMap() : AbstractMap<float>() {
  m_map = NULL;
  m_defaultCell = -1;
}
...
bool CarmenMap::init(const MapConfig& cfg) {
  ...
  m_map->complete_map = new float[cfg.m_sizeX*cfg.m_sizeY];
  carmen_test_alloc(m_map->complete_map);
  m_map->map = new float*[cfg.m_sizeX];
...
CarmenMap::~CarmenMap() {
  if (AbstractMap<float>::m_cfg.isValid() && m_map != NULL)
