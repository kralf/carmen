...
static BOOLEAN x_ipc_handleQueryNotification (DATA_MSG_PTR dataMsg)
{
  ...
  if (queryNotif) {
    ...
    queryNotif->ref->encoding.byteOrder = dataMsg->dataByteOrder;
    queryNotif->ref->encoding.alignment = dataMsg->alignment;
              ...
              X_IPC_RETURN_VALUE_TYPE *status)
{
  ...
  if (dataMsg->intent == QUERY_REPLY_INTENT) {
    if (ref != NULL) {
      ref->encoding.byteOrder = dataMsg->dataByteOrder;
      ref->encoding.alignment = dataMsg->alignment;
    }
