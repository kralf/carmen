...
static void
range_to_xy(carmen_robot_laser_message *laser, GdkPoint *laser_poly,
	    double *laser_dist)
{
  int i;
  double theta;
...
static void
update_polygons(int received_laser, carmen_robot_laser_message *laser_msg,
...
static void
draw_robot_sonar(GdkPixmap *pixmap, carmen_robot_sonar_message *sonar_msg)
{
...
static void
draw_robot(GdkPixmap *pixmap, carmen_robot_laser_message *laser_msg)
{
  GdkPoint rect_robot[4];
  GdkPoint p1, p2;
  GdkPoint top_left;
  ...
  if (show_velocity &&
      fabs(laser_msg->tv) < 0.001 && fabs(laser_msg->rv) > .00001)
    {
      ...
      gdk_draw_arc(pixmap, drawing_gc, FALSE, width_2 - length/2.0*scale*1.1,
	   ...
	   fabs(laser_msg->tv) > 0.001 && fabs(laser_msg->rv) > 0.001)
    {
      gdk_gc_set_foreground(drawing_gc, &carmen_green);
...
redraw(void)
{
  ...
  if (!received_front_laser && !received_rear_laser &&
      !received_sonar && font != NULL)
    {
      gdk_gc_set_foreground(drawing_gc, &carmen_white);
