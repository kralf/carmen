...
static void image_handler(carmen_camera_image_message *image_msg)
{
  ...
  if ((image_msg->bytes_per_pixel != 1) && (image_msg->bytes_per_pixel != 3))
    return;

  gtk_window_get_size(GTK_WINDOW(main_window), &width, &height);
  if ((width != image_msg->width) || (height != image_msg->height))
    gtk_window_resize(GTK_WINDOW(main_window), image_msg->width,
    image_msg->height);
  if (received_image)
    g_object_unref(image);

  data = (unsigned char *)calloc(image_msg->width*image_msg->height*3,
    sizeof(unsigned char));
  carmen_test_alloc(data);

  if (image_msg->bytes_per_pixel == 1) {
    for (i = 0; i < image_msg->image_size; i++) {
      data[i*3] = image_msg->image[i];
      data[i*3+1] = image_msg->image[i];
      data[i*3+2] = image_msg->image[i];
    }
  }
  else {
    for (i = 0; i < image_msg->image_size; i++)
      data[i] = image_msg->image[i];
  }

  image = gdk_pixbuf_new_from_data((guchar*)data, GDK_COLORSPACE_RGB,
    FALSE, 8, image_msg->width, image_msg->height, image_msg->width*3,
    pixbuf_destroyed, NULL);

  char window_title[256];
  sprintf(window_title, "Camera View <%dx%dx%dbpp>",
    image_msg->width, image_msg->height, image_msg->bytes_per_pixel*8);
...
start_graphics(int argc, char *argv[])
{
  ...
  gtk_window_set_title(GTK_WINDOW(main_window), "Camera View <empty>");

  drawing_area = gtk_drawing_area_new ();
  gtk_window_resize(GTK_WINDOW(main_window), 640, 480);
