...
float expected_laser[MAX_SCANS_PER_SCAN];
...
static void redraw(void)
{
  ...
  /* draw the robot */
  x = (sensor.pose.x - param.front_laser_offset * cos(sensor.pose.theta)) /
    map.config.resolution;
  y = height - (sensor.pose.y - param.front_laser_offset * 
...
void localize_sensor_handler(void)
{
  ...
  compute_expected_scan(&map.carmen_map, pose.x, pose.y, pose.theta,
    ...
    sensor.config.angular_resolution, sensor.config.start_angle);

  incorporate_laser_difference(sensor.range, expected_laser, 
    sensor.num_readings,
    sensor.config.angular_resolution, sensor.config.start_angle,
    pose.x, pose.y, pose.theta, &mod_map);
...
void read_parameters(int argc, char **argv)
{
  ...
  carmen_param_t param_list[] = {
    ...
    {"localize", "lmap_std", CARMEN_PARAM_DOUBLE, &param.lmap_std, 0, NULL},
