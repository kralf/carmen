...
static int initialize_arm(void)
{
  ...
  result = carmen_arm_direct_initialize(&arm);
  if (result < 0) 
    return -1;
...
static int read_parameters(int argc, char **argv)
{
  ...
  carmen_param_t param_list[] = {
    { "arm", "dev", CARMEN_PARAM_STRING, &arm.dev, 0, NULL},
    { "arm", "num_joints", CARMEN_PARAM_INT, &arm.num_joints, 0, NULL },
  };
			      ...
			      void *clientData __attribute__ ((unused)))
{
  ...
  strcpy(response.host, carmen_get_host());
			  ...
			  void *clientData __attribute__ ((unused)))
{
  ...
  formatter = IPC_msgInstanceFormatter(msgRef);
  err = IPC_unmarshallData(formatter, callData, &msg,
			   sizeof(carmen_base_reset_command_message));
  IPC_freeByteArray(callData);
